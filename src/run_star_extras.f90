...
! ***********************************************************************
      ...
      subroutine extras_controls(id, ierr)
         ...
         s% other_energy => summerschool_energy_routine
      ...
      ! x_ctrls from the inlist
      ! x_ctrl(1): Mtransition in Msol units
      ! x_ctrl(2): high entropy
      ! x_ctrl(3): low entropy
      ! x_ctrl(4): etatransition
      subroutine summerschool_energy_routine(id, ierr)
         use const_def, only: Msun
         integer, intent(in) :: id
         integer, intent(out) :: ierr
         type (star_info), pointer :: s
         integer :: k
         double precision :: trel, mtransition, etatransition
         double precision :: highentropy, lowentropy, tempWD

         ! get a pointer to the star data
         ierr = 0
         call star_ptr(id, s, ierr)
         if (ierr /= 0) return
         ...
         do k = 1, s% nz
            ! this sets the region outside of
            ! mtransition to a high constant entropy
            ! this mimics a disrupted WD companion that has been
            ! added to the undisrupted WD as a virialized envelope
            highentropy = s%x_ctrl(2)
            if (s% m(k) > mtransition) then
               s% extra_heat(k) = ( highentropy - exp(s%lnS(k)) )*s%T(k)/trel
            end if

            ! if the inner region is not degenerate "enough" (eta < etatransition),
            ! cool it down to a low constant entropy
            lowentropy = s%x_ctrl(3)
            etatransition = s%x_ctrl(4)
            if (s% m(k) .le. mtransition .and. s%eta(k) < etatransition) then
               s% extra_heat(k) = ( lowentropy - exp(s%lnS(k)) )*s%T(k)/trel
            end if

            ! when the inner region becomes degenerate "enough" (eta > etatransition),
            ! make it isothermal, with temperature = tempWD
            tempWD = 3d7
            if (s% m(k) .le. mtransition .and. s%eta(k) .ge. etatransition) then
               s% extra_heat(k) = ( tempWD - s%T(k) )*exp(s%lnS(k))/trel
            end if
         end do

      end subroutine summerschool_energy_routine
